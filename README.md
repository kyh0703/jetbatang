...
한글은 RIDIBatang 에서 가져옵니다. 한글은 라틴 1칸의 정확히 두 배 advance 에
맞춰 넣기 때문에 터미널 격자가 어긋나지 않습니다.
...
- **Regular 아래 굵기의 한글은 가늘어지지 않습니다.** RIDIBatang 이 한 굵기만
  제공합니다. 획을 불릴 수는 있어도 깎을 수는 없습니다.
- **굵은 쪽 한글은 가짜 볼드**입니다. 원본 디자이너가 그린 굵기가 아니라 같은
  외곽선을 겹쳐 불린 것입니다. 13px 아래에서는 `뷁` 같은 복잡한 글자의 속공간이
  메워집니다.
